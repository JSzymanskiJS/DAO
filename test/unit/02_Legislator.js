...
const GOVERNOR_ARTIFACTS = require("../../artifacts/contracts/Governance/Governor.sol/Governor.json");

describe("Test Legislator", function () {
    let owner;
    let addr1;
    let addr2;
    let addrs;
    ...
    beforeEach(async () => {
        ...
        const Legislator = await ethers.getContractFactory("Legislator", {
            libraries: {
                StringManipulation: stringManipulation.address
            }
        });
        legislator = await Legislator.deploy(wETH.address);
        await legislator.deployed();
        governorAddress_Tx = await legislator.governor();
        governor = new ethers.Contract(governorAddress_Tx, GOVERNOR_ARTIFACTS.abi, owner);
    });

    describe("Test constructor()", function () {
        it("PASS", async () => {
            expect(await legislator.governor()).to.equal(governor.address);
        });
    });
    ...
    describe("Test createProposal()", function () {
        it("PASS", async () => {
            ...
            createProposal_Tx = await legislator.connect(owner).createProposal(
                ETHER[10],
                date,
                date + 60,
                70,
        ...
        it("FAIL - Wrong address", async () => {
            date = Math.floor(Date.now() / 1000);
            await expect(legislator.connect(addr1).createProposal(
                ETHER[10],
                date,
                date + 60,
                70,
        ...
        it("FAIL - Pass percentage < rejection percentage", async () => {
            date = Math.floor(Date.now() / 1000);
            await expect(legislator.connect(owner).createProposal(
                ETHER[10],
                date,
                date + 60,
                30,
    ...
    describe("Test setMinVotesAmount()", function () {
        it("PASS", async () => {
            ...
            changeMinVotes_Tx = await legislator.setMinVotesAmount(1, ETHER[11]);
            proposal = await governor.idToProposal(1);
        ...
        it("FAIL - Wrong Address", async () => {
            ...
            await expect(legislator.connect(addr1).setMinVotesAmount(1, ETHER[11])).to.be.revertedWith("Ownable: caller is not the owner");
        });
    });
    ...
    describe("Test setMinPassPercentage()", function () {
        it("PASS", async () => {
            ...
            changeMinVotes_Tx = await legislator.setMinPassPercentage(1, 40);
            proposal = await governor.idToProposal(1);
        ...
        it("FAIL - Wrong Address", async () => {
            ...
            await expect(legislator.connect(addr1).setMinPassPercentage(1, 90)).to.be.revertedWith("Ownable: caller is not the owner");
        });
    });
    ...
    describe("Test setMaxRejectedPercentage()", function () {
        it("PASS", async () => {
            ...
            changeMinVotes_Tx = await legislator.setMaxRejectedPercentage(1, 20);
            proposal = await governor.idToProposal(1);
            expect(proposal.abstainVotes).to.equal(ETHER[25]);
            expect(proposal.result).to.equal(enumResult["REJECTED"]);
        });
        ...
        it("FAIL - Wrong Address", async () => {
            ...
            await expect(legislator.connect(addr1).setMaxRejectedPercentage(1, 60)).to.be.revertedWith("Ownable: caller is not the owner");
        });
    });
    ...
    describe("Test setStartTimestamp()", function () {
        it("PASS", async () => {
            ...
            changeMinVotes_Tx = await legislator.setStartTimestamp(1, date + 500);
            proposal = await governor.idToProposal(1);
        ...
        it("FAIL - Wrong Address", async () => {
            ...
            await expect(legislator.connect(addr1).setStartTimestamp(1, 60)).to.be.revertedWith("Ownable: caller is not the owner");
        });
    });
    ...
    describe("Test setEndTimestamp()", function () {
        it("PASS", async () => {
            ...
            changeMinVotes_Tx = await legislator.setEndTimestamp(1, date + 5);
            proposal = await governor.idToProposal(1);
        ...
        it("FAIL - Wrong Address", async () => {
            ...
            await expect(legislator.connect(addr1).setEndTimestamp(1, 60)).to.be.revertedWith("Ownable: caller is not the owner");
        });
    });
});